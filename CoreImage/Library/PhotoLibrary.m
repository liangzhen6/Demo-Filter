...
+ (void)writePhotoWithImage:(UIImage *)image result:(void(^)(BOOL res))result {
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        PHAssetCreationRequest * request = [PHAssetCreationRequest creationRequestForAsset];
...
//    imagePick.mediaTypes = @[@"public.movie"];//只获取视频数据
    imagePick.mediaTypes = @[@"public.image"];//只获取图片数据
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    //是图片
    if ([[info[@"UIImagePickerControllerImageURL"] absoluteString] length]) {
        if (self.result) {
            self.result(info[@"UIImagePickerControllerImageURL"], YES);
        }
    }
}
